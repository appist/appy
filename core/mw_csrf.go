...
func getCSRFTokenFromCookie(ctx *Context, config AppConfig) ([]byte, error) {
...
func getCSRFTokenFromRequest(ctx *Context, config AppConfig) []byte {
	r := ctx.Request
...
func saveCSRFTokenIntoCookie(token []byte, ctx *Context, config AppConfig) error {
