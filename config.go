...
type (
	...
	Config struct {
		// AppyEnv indicates the environment that the codebase is running on and it determines which
		// config to use. By default, it is is "development" and its corresponding config is
		// "configs/.env.development".
		//
		// Note: APPY_ENV=test is used for unit tests.
		AppyEnv string `env:"APPY_ENV" envDefault:"development"`

		// AssetHost indicates the asset host to use with "assetPath()" for the server-side rendering
		// which is very useful it comes to hosting the static assets on CDN. By default, it is ""
		// which uses the current server host.
		AssetHost string `env:"ASSET_HOST" envDefault:""`

		// GQLPlaygroundEnabled indicates if the GraphQL playground is enabled. By default, it is
		// false.
		GQLPlaygroundEnabled bool `env:"GQL_PLAYGROUND_ENABLED" envDefault:"false"`

		// GQLPlaygroundPath indicates the GraphQL playground path to host at. By default, it is
		// "/docs/graphql".
		GQLPlaygroundPath string `env:"GQL_PLAYGROUND_PATH" envDefault:"/docs/graphql"`

		// GQLAPQCacheSize indicates how many APQ to persist in the memory at one time. By default, it
		// is 100. For more details about APQ, please refer to https://gqlgen.com/reference/apq.
		...
		// GQLComplexityLimit indicates the query complexity which can be used to mitigate DDoS attacks
		// risk. By default, it is 1000.
		GQLComplexityLimit int `env:"GQL_COMPLEXITY_LIMIT" envDefault:"1000"`

		// GQLMultipartMaxMemory indicates the maximum number of bytes used to parse a request body as
		// multipart/form-data in memory, with the remainder stored on disk in temporary files. By
		// default, it is 0 (no limit).
		GQLMultipartMaxMemory int64 `env:"GQL_MULTIPART_MAX_MEMORY" envDefault:"0"`

		// GQLMultipartMaxUploadSize indicates the maximum number of bytes used to parse a request body
		// as multipart/form-data. By default, it is 0 (no limit).
		GQLMultipartMaxUploadSize int64 `env:"GQL_MULTIPART_MAX_UPLOAD_SIZE" envDefault:"0"`

		// GQLWebsocketKeepAliveDuration indicates how long the websocket connection should be kept
		// alive for sending subsequent messages without re-establishing the connection which is an
		// overhead. By default, it is 10s.
		GQLWebsocketKeepAliveDuration time.Duration `env:"GQL_WEBSOCKET_KEEP_ALIVE_DURATION" envDefault:"10s"`

		// HTTPGzipCompressLevel indicates the compression level used to compress the HTTP response.
		// By default, it is -1.
		//
		// Available options:
		...
		// HTTPLogFilterParameters indicates which query parameters in the URL to filter so that the
		// sensitive information like password are masked in the HTTP request log. By default, it is
		// "password".
		HTTPLogFilterParameters []string `env:"HTTP_LOG_FILTER_PARAMETERS" envDefault:"password"`

		// HTTPHealthCheckURL indicates the path to check if the HTTP server is healthy. This endpoint
		// is a middleware that is designed to avoid redundant computing resource usage. By default,
		// it is "/health_check".
		//
		// In general, if your server is running behind a load balancer, this endpoint will be served to
		// inform the load balancer that the server is healthy and ready to receive HTTP requests.
		HTTPHealthCheckURL string `env:"HTTP_HEALTH_CHECK_URL" envDefault:"/health_check"`

		// HTTPHost indicates which host the HTTP server should be hosted at. By default, it is
		// "localhost". If you would like to connect to the HTTP server from within your LAN network,
		// use "0.0.0.0" instead.
		...
		// HTTPGracefulTimeout indicates how long to wait for the HTTP server to shut down so that any
		// active connection is not interrupted by SIGTERM/SIGINT. By default, it is "30s".
		HTTPGracefulTimeout time.Duration `env:"HTTP_GRACEFUL_TIMEOUT" envDefault:"30s"`

		// HTTPIdleTimeout is the maximum amount of time to wait for the next request when keep-alives
		// are enabled. If HTTPIdleTimeout is zero, the value of HTTPReadTimeout is used. If both are
		// zero, there is no timeout. By default, it is "75s".
		HTTPIdleTimeout time.Duration `env:"HTTP_IDLE_TIMEOUT" envDefault:"75s"`

		// HTTPMaxHeaderBytes controls the maximum number of bytes the server will read parsing the
		// request header's keys and values, including the request line. It does not limit the size of
		// the request body. If zero, http.DefaultMaxHeaderBytes (1 << 20 which is 1 MB) is used.
		...
		// Because HTTPReadTimeout does not let Handlers make per-request decisions on each request
		// body's acceptable deadline or upload rate, most users will prefer to use
		// HTTPReadHeaderTimeout. It is valid to use them both. By default, it is "60s".
		HTTPReadTimeout time.Duration `env:"HTTP_READ_TIMEOUT" envDefault:"60s"`

		// HTTPReadHeaderTimeout is the amount of time allowed to read request headers. The connection's
		// read deadline is reset after reading the headers and the Handler can decide what is
		// considered too slow for the body. If HTTPReadHeaderTimeout is zero, the value of HTTPReadTimeout
		// is used. If both are zero, there is no timeout. By default, it is "60s".
		HTTPReadHeaderTimeout time.Duration `env:"HTTP_READ_HEADER_TIMEOUT" envDefault:"60s"`

		// HTTPWriteTimeout is the maximum duration before timing out writes of the response. It is
		// reset whenever a new request's header is read. Like HTTPReadTimeout, it does not let
		// Handlers make decisions on a per-request basis. By default, it is "60s".
		HTTPWriteTimeout time.Duration `env:"HTTP_WRITE_TIMEOUT" envDefault:"60s"`

		// HTTPSSLCertPath indicates which path to store the locally trusted SSL certificates which are
		// created using "go run . ssl:setup" command. By default, it is "./tmp/ssl".
		HTTPSSLCertPath string `env:"HTTP_SSL_CERT_PATH" envDefault:"./tmp/ssl"`

		// HTTPSSLEnabled indicates if the HTTPS server should be enabled. When enabled, please ensure
		// the SSL certificates are created in the "HTTPSSLCertPath". By default, it is false.
		HTTPSSLEnabled bool `env:"HTTP_SSL_ENABLED" envDefault:"false"`

		// HTTPSSLPort indicates which port the HTTPS server should be hosted at. By default, it is
		// "3443".
		HTTPSSLPort string `env:"HTTP_SSL_PORT" envDefault:"3443"`

		// HTTPSessionProvider indicates which store to use for storing the session information. By
		// default, it is "cookie".
		//
		// Available options:
		//   - cookie
		//   - redis
		...
		// HTTPSessionExpiration indicates how long till the session should expire. By default,
		// it is 1209600 in seconds.
		//
		// Note: If the HTTPSessionProvider is "redis", the same value will be used to expire the
		// session in Redis.
		HTTPSessionExpiration int `env:"HTTP_SESSION_EXPIRATION" envDefault:"1209600"`

		// HTTPSessionSecrets indicates the secrets to encrypt the session information. By default,
		// it is "".
		//
		// Note: Multiple values are accepted with comma delimited to allow easier secret rotation.
		HTTPSessionSecrets [][]byte `env:"HTTP_SESSION_SECRETS,required" envDefault:""`

		// HTTPSessionRedisAddr indicates the Redis server to store the session. The Redis session
		// store will store the session's information and use cookie to store the session ID. By
		// default, it is "localhost:6379".
		//
		// Note: Please ensure that HTTPSessionProvider is configured to be "redis" when using this.
		HTTPSessionRedisAddr string `env:"HTTP_SESSION_REDIS_ADDR" envDefault:"localhost:6379"`

		// HTTPSessionRedisAuth indicates the password to authenticate with the Redis server. By
		// default, it is "".
		...
		// connection pool at one time. By default, it is 32.
		HTTPSessionRedisMaxIdle int `env:"HTTP_SESSION_REDIS_MAX_IDLE" envDefault:"32"`

		// HTTPSessionRedisIdleTimeout indicates how long the idle connection should be kept around. If
		// it is 0, the idle connections won't be closed. By default, it is "30s".
		HTTPSessionRedisIdleTimeout time.Duration `env:"HTTP_SESSION_REDIS_IDLE_TIMEOUT" envDefault:"30s"`

		// HTTPSessionRedisMaxConnLifetime indicates how long a connection should be kept alive. If it
		// is 0, no connections will be closed based on the age. By default, it is "30s".
		HTTPSessionRedisMaxConnLifetime time.Duration `env:"HTTP_SESSION_REDIS_MAX_CONN_LIFETIME" envDefault:"30s"`

		// HTTPSessionRedisWait indicates whether or not to wait for the connection pool to return a
		// usable connection when it is at its HTTPSessionRedisMaxActive limit. By default, it is true.
		HTTPSessionRedisWait bool `env:"HTTP_SESSION_REDIS_WAIT" envDefault:"true"`

		// HTTPSessionCookieName indicates the cookie name to use to store the session. By default, it
		// is "_session".
		HTTPSessionCookieName string `env:"HTTP_SESSION_COOKIE_NAME" envDefault:"_session"`

		// HTTPSessionCookieDomain indicates which domain the session cookie can be sent to. By
		...
		// HTTPSessionCookieSecure indicates if the session cookie can only be sent with HTTPS request.
		// By default, it is false.
		HTTPSessionCookieSecure bool `env:"HTTP_SESSION_COOKIE_SECURE" envDefault:"false"`

		// HTTPAllowedHosts indicates a list of fully qualified domain names that are allowed to be
		// processed by the HTTP server. By default, it is "" which allows all domain names.
		HTTPAllowedHosts []string `env:"HTTP_ALLOWED_HOSTS" envDefault:""`

		// HTTPCSRFCookieDomain indicates which domain the CSRF cookie can be sent to. By default, it
		// is "localhost".
		HTTPCSRFCookieDomain string `env:"HTTP_CSRF_COOKIE_DOMAIN" envDefault:"localhost"`

		// HTTPCSRFCookieName indicates the cookie name to use to store the CSRF token. By default, it
		// is "_csrf_token".
		HTTPCSRFCookieName string `env:"HTTP_CSRF_COOKIE_NAME" envDefault:"_csrf_token"`

		// HTTPCSRFCookieHTTPOnly indicates if the CSRF cookie is only accessible via HTTP request and
		// not Javascript `Document.cookie` API. By default, it is true.
		HTTPCSRFCookieHTTPOnly bool `env:"HTTP_CSRF_COOKIE_HTTP_ONLY" envDefault:"true"`

		// HTTPCSRFCookieMaxAge indicates how long till the CSRF cookie should expire. By default, it
		// is 0 which is no expiration.
		HTTPCSRFCookieMaxAge int `env:"HTTP_CSRF_COOKIE_MAX_AGE" envDefault:"0"`

		// HTTPCSRFCookiePath indicates which URL path the CSRF cookie can be sent to. By default, it
		// is "/".
		HTTPCSRFCookiePath string `env:"HTTP_CSRF_COOKIE_PATH" envDefault:"/"`

		// HTTPCSRFCookieSameSite indicates if the CSRF cookie can be sent with cross-site requests.
		// By default, it is 1.
		//
		// Available options:
		//   - SameSiteDefaultMode = 1
		...
		// HTTPCSRFCookieSecure indicates if the session cookie can only be sent with HTTPS request.
		// By default, it is false.
		HTTPCSRFCookieSecure bool `env:"HTTP_CSRF_COOKIE_SECURE" envDefault:"false"`

		// HTTPCSRFFieldName indicates the POST form field name that contains the authenticity token
		// for CSRF check. By default, it is "authenticity_token".
		HTTPCSRFFieldName string `env:"HTTP_CSRF_FIELD_NAME" envDefault:"authenticity_token"`

		// HTTPCSRFRequestHeader indicates the HTTP header that contains the authenticity token for
		// CSRF check. By default, it is "X-CSRF-Token".
		HTTPCSRFRequestHeader string `env:"HTTP_CSRF_REQUEST_HEADER" envDefault:"X-CSRF-Token"`

		// HTTPCSRFSecret indicates the secret to encrypt the CSRF cookie. By default, it is "".
		HTTPCSRFSecret []byte `env:"HTTP_CSRF_SECRET,required" envDefault:""`

		// HTTPSSLRedirect indicates if the HTTP server should automatically redirect HTTP requests to
		// HTTPS. By default, it is false.
		HTTPSSLRedirect bool `env:"HTTP_SSL_REDIRECT" envDefault:"false"`

		// HTTPSSLTemporaryRedirect indicates if the HTTPSSLRedirect should use 302 temporary redirect.
		// By default, it is false which uses 301 permanent redirect.
		HTTPSSLTemporaryRedirect bool `env:"HTTP_SSL_TEMPORARY_REDIRECT" envDefault:"false"`

		// HTTPSSLHost indicates the host name to redirect to when redirecting HTTP requests to HTTPS.
		// By default, it is "" which uses the same host.
		HTTPSSLHost string `env:"HTTP_SSL_HOST" envDefault:""`

		// HTTPSSLProxyHeaders indicates the SSL proxy headers to verify when the server is running
		// behind a proxy like Heroku to ensure the request comes through the SSL proxy server. By
		// default, it is "X-Forwarded-Proto:https".
		HTTPSSLProxyHeaders map[string]string `env:"HTTP_SSL_PROXY_HEADERS" envDefault:"X-Forwarded-Proto:https"`

		// HTTPSTSSeconds indicates the max-age of the "Strict-Transport-Security" response header. By
		// default, it is 0 which would NOT include the header.
		HTTPSTSSeconds int64 `env:"HTTP_STS_SECONDS" envDefault:"0"`

		// HTTPSTSIncludeSubdomains indicates if "includeSubdomains" should be appended to the
		// "Strict-Transport-Security" response header. By default, it is false.
		HTTPSTSIncludeSubdomains bool `env:"HTTP_STS_INCLUDE_SUBDOMAINS" envDefault:"false"`

		// HTTPFrameDeny indicates if the page cannot be contained in an iframe by setting the
		// "X-Frame-Options: DENY" response header. By default, it is false.
		HTTPFrameDeny bool `env:"HTTP_FRAME_DENY" envDefault:"false"`

		// HTTPCustomFrameOptionsValue indicates the custom value to be set for the "X-Frame-Options"
		// response header. By default, it is "" which doesn't override the "DENY" value.
		HTTPCustomFrameOptionsValue string `env:"HTTP_CUSTOM_FRAME_OPTIONS_VALUE" envDefault:""`

		// HTTPContentTypeNosniff indicates if the "X-Content-Type-Options: nosniff" response header
		// should be set. By default, it is false.
		HTTPContentTypeNosniff bool `env:"HTTP_CONTENT_TYPE_NOSNIFF" envDefault:"false"`

		// HTTPBrowserXSSFilter indicates if the "X-XSS-Protection: 1; mode=block" response header
		// should be set. By default, it is false.
		HTTPBrowserXSSFilter bool `env:"HTTP_BROWSER_XSS_FILTER" envDefault:"false"`

		// HTTPContentSecurityPolicy indicates the custom value to be set for "Content-Security-Policy"
		// response header. By default, it is "".
		HTTPContentSecurityPolicy string `env:"HTTP_CONTENT_SECURITY_POLICY" envDefault:""`

		// HTTPReferrerPolicy indicates the referrer information that is sent in the "Referrer" header
		// to be included with the requests made. By default, it is "".
		HTTPReferrerPolicy string `env:"HTTP_REFERRER_POLICY" envDefault:""`

		// HTTPIENoOpen indicates if it should prevent Internet Explorer from executing downloads in
