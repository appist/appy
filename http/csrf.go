...
func CSRF(config *support.Config, logger *support.Logger) HandlerFunc {
...
func csrfHandler(c *Context, config *support.Config, logger *support.Logger) {
	...
	err = saveAuthenticityTokenIntoCookie(authenticityToken, c, config)
	if err != nil {
		logger.Error(err)
		c.AbortWithError(http.StatusForbidden, err)
		return
...
func saveCSRFTokenIntoCookie(token []byte, c *Context, config *support.Config) error {
...
func saveAuthenticityTokenIntoCookie(token string, c *Context, config *support.Config) error {
	c.SetCookie(
