...
func NewServer(asset *Asset, config *Config, logger *Logger, support Supporter) *Server {
	router := newRouter()
...
func (s *Server) Hosts() ([]string, error) {
	...
	if s.config.HTTPHost != "" && !s.support.ArrayContains(hosts, s.config.HTTPHost) {
