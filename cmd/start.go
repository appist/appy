...
func NewStartCommand(s core.AppServer) *AppCmd {
	return &AppCmd{
		Use:   "start",
...
func runWebServeCmd(s core.AppServer) {
	wd, _ := os.Getwd()
	ssrPaths := []string{}
...
func watch(s core.AppServer, watchPaths []string, callback func(e watcher.Event)) {
	w := watcher.New()
	defer w.Close()
